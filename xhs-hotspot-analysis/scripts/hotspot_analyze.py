...
def build_prompt(notes: list[dict], framework: str, category: str, extra_context: str = "", target_category: str = "") -> str:
    ...
    if target_category and target_category.strip() != category:
        extra += (
            ...
            "在品类级总结之后，额外输出一节「跨品类迁移建议」：分析这批爆款的【钩子】【结构】【情绪】【人群】"
            f"哪些可以迁移到目标品类「{target_category}」，给出 2-3 个具体迁移示例（含改写后的标题示例、套用同一结构的新选题）。\n"
        )
...
# 任务
1. 按框架的「单条笔记拆解」维度，对每条候选给出结论（证据来自上面 JSON）。
2. 按框架的「单条评分」给出每条 score 和 tier。
3. 按框架的「品类级总结」输出四段：热点分布 / 爆款公式 / 空白机会 / 可复刻选题。
4. 输出为 Markdown，直接可作为 analysis.md。
5. 注意长度：单条拆解每条约 4-6 行要点即可，把篇幅留给"品类级总结"（这是报告最有价值的部分）。务必完整输出全部 {len(notes)} 条拆解 + 四段总结，不要截断。
{extra}"""
